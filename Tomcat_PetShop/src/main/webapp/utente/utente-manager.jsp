...
<script type="text/javascript">
    // Funzione per abilitare o disabilitare l'utente
    function changeUserStatus(userId, newStatus) {
        $.ajax({
            type: "POST",
            url: "cambiaStatoUtente.jsp",
            data: {
                userId: userId,
                newStatus: newStatus
            },
            success: function(response) {
                // Ricarica la pagina dopo l'aggiornamento per visualizzare i cambiamenti
                location.reload();
            },
            error: function(xhr, status, error) {
                console.log("Errore durante l'aggiornamento dello stato dell'utente.");
            }
        });
    }
</script>
		...
		<table border="1" width="90%">
			...
			<c:forEach items="${sublist}" var="u">
				<tr>
					...
					<td>${u.getRuolo()}</td>
					<td><c:choose>
							<c:when test="${u.getStato() == 'A'}">
								<!-- Se lo stato è 'A' (Attivo), mostra il pulsante Disabilita -->
								<button class="disabled"
									onclick="changeUserStatus('${u.getId()}', 'D')">Disabilita</button>
							</c:when>
							<c:otherwise>
								<!-- Altrimenti, mostra il pulsante Abilita -->
								<button onclick="changeUserStatus('${u.getId()}', 'A')">Abilita</button>
							</c:otherwise>
						</c:choose></td>
					<td><form action="modifica-utente.jsp" method="post">
